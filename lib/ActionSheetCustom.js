...
class ActionSheet extends React.Component {
  ...
  _renderCancelButton() {
    ...
    return this._createButton(options[cancelButtonIndex], cancelButtonIndex, null, {
  ...
  _createButton(title, index, length, containerStyle = {}) {
    const styles = this.styles
    ...
    const radius = length && index === length - 2 ? 10 : 0
    return (
      <TouchableHighlight
        key={index}
        activeOpacity={1}
        underlayColor={buttonUnderlayColor}
        style={[buttonBoxStyle, { borderBottomLeftRadius: radius, borderBottomRightRadius: radius }, containerStyle]}
  ...
  _renderOptions() {
    const { cancelButtonIndex } = this.props
    return this.props.options.map((title, index, array) => {
      return cancelButtonIndex === index ? null : this._createButton(title, index, array.length)
    })
  }
